...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = langs.map(
    lang => lang -> occurrencesOfLang(lang,rdd)).sortBy(- _._2)

  /* Compute an inverted index of the set of articles, mapping each language
   * to the Wikipedia pages in which it occurs.
   */
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = rdd.flatMap(w =>
    langs.withFilter(lang => w.mentionsLanguage(lang)).map(lang => lang -> w)
  ).groupByKey()
   ...
   *   several seconds.
   */
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = index.map(v => v._1 -> v._2.size)
   ...
   *   several seconds.
   */
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = rdd.flatMap(w =>
    langs.withFilter(lang => w.mentionsLanguage(lang)).map(lang => lang -> 1)
